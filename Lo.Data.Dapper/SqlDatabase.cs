...
using System.Text;
using Dapper;
using Lo.Data.Attributes;
using Lo.Data.Extension;
using Lo.Util;
using SQLinq;
using SQLinq.Dapper;

namespace Lo.Data.Dapper
{
    public class SqlDatabase : IDatabase
    {
        ...
        public int Delete<T>(Expression<Func<T, bool>> condition) where T : class, new()
        {
            ...
            var table = EntityAttribute.GetEntityTable<T>();
            IEnumerable<T> entities = dbTransaction.Connection.Query<T>(new SQLinq<T>(table).Where(condition));
            Delete<T>(entities);
            if (!isTrans)
            {
                return Commit();
        ...
        public int Delete<T>(object propertyValue, string propertyName) where T : class
        {
            ...
            string querySql = $"select * from {EntityAttribute.GetEntityTable<T>()} where {propertyName}=@propertyValue";
            IEnumerable<T> entitys = dbTransaction.Connection.Query<T>(querySql, new { propertyValue = propertyValue });
        ...
        public T FindEntity<T>(object keyValue) where T : class
        {
            ...
            {
                string querySql = $"select * from {EntityAttribute.GetEntityTable<T>()} where key=@key";
                var data = dbConnection.Query<T>(querySql, new { key = keyValue });
            ...
            int pageIndex, out int total) where T : class
        {
            using (var dbConnection = Connection)
            {
                ...
                return ConvertExtension.DataReaderToList<T>(IDataReader);
            }
        }
        #endregion
        ...
        public DataTable FindTable(string strSql, DbParameter[] dbParameter)
        {
            using (var dbConnection = Connection)
            {
                ...
                return ConvertExtension.DataReaderToDataTable(IDataReader);
            }
        }
            ...
            int pageIndex, out int total)
        {
            using (var dbConnection = Connection)
            {
                ...
                total = Convert.ToInt32(new DbHelper(dbConnection).ExecuteScalar(CommandType.Text, "Select Count(1) From (" + strSql + ") As t", dbParameter));
                var IDataReader = new DbHelper(dbConnection).ExecuteReader(CommandType.Text, sb.ToString(), dbParameter);
                DataTable resultTable = ConvertExtension.DataReaderToDataTable(IDataReader);
